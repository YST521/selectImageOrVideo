...
/*
 * if Use ALAsset (NS_CLASS_DEPRECATED_IOS(4_0, 9_0)) on iOS 8.0+ only modify to < 10.0
 */
#define MIS_IMAGE_PICKER_SUPPORT_IOS7 ([UIDevice currentDevice].systemVersion.floatValue < 8.0)
...
- (void)registerNotifacations {
	...
	//系统相册数据变动时，重置相册数据
	if (MIS_IMAGE_PICKER_SUPPORT_IOS7) {
...
- (void)fetchAlbumsWithCompletion:(void(^)(void))completion {
	if (MIS_IMAGE_PICKER_SUPPORT_IOS7) {
		[self fetchAlbumsInALAssetsLibraryWithCompletion:^(NSArray *albums) {
 ...
 *  从ALAsset中取相册(iOS7.0)
 */
- (void)fetchAlbumsInALAssetsLibraryWithCompletion:(void(^)(NSArray* albums))completion {
	if (!completion) {
		return;
	}
	//开线程运行
...
- (void)fetchImageForAsset:(id)asset
	...
	if ([asset isKindOfClass:[ALAsset class]]) {
		...
		completion(image);//先返回小图
		
		dispatch_async(dispatch_get_global_queue(0, 0), ^{
			ALAssetRepresentation* representation = [alAsset defaultRepresentation];
			UIImage* image = [UIImage imageWithCGImage:representation.fullScreenImage];
			dispatch_async(dispatch_get_main_queue(), ^{
				completion(image);//再返回大图
			});
		});
	}else if ([asset isKindOfClass:[PHAsset class]]) { //PHAsset for iOS 8.0+
	...
	dispatch_async(dispatch_get_global_queue(0, 0), ^{
		...
		for (id asset in assets) {
			...
			if ([asset isKindOfClass:[ALAsset class]]) {
				...
				//取屏幕大小的图片
				ALAssetRepresentation* representation = alAsset.defaultRepresentation;
				originImage = [UIImage imageWithCGImage:representation.fullScreenImage];
 ...
 *  @param completion 结果
 */
- (void)albumAuthorizationWithAvailableCompletion:(void(^)(void))completion {
	if (!MIS_IMAGE_PICKER_SUPPORT_IOS7) {
		[PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
			switch (status) {
				...
				{
					dispatch_async(dispatch_get_main_queue(), ^{
						[self showAlbumUnAuthorizedAlert];
					});
				}
					break;
 ...
 * 照片未授权提示
 */
- (void)showAlbumUnAuthorizedAlert {
	NSString* appName = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleDisplayName"];
	NSString* message = [NSString stringWithFormat:@"请在iPhone的\"设置-稳私-照片\"选项中，允许%@访问你的手机相册。", appName];
...
- (void)showCameraUnAuthorizedAlert {
	NSString* appName = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleDisplayName"];
	NSString* message = [NSString stringWithFormat:@"请在iPhone的\"设置-稳私-相机\"选项中，允许%@访问你的相机。", appName];
	UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示"
												   message:message
												  delegate:nil
										 cancelButtonTitle:@"确定"
 ...
 *  @param completion 结果
 */
- (void)cameraAuthorizationWithAvailableCompletion:(void(^)(void))completion {
	...
	switch (authStatus) {
		...
		{
			dispatch_async(dispatch_get_main_queue(), ^{
				[self showCameraUnAuthorizedAlert];
			});
		}
			break;
